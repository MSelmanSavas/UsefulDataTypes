...
public class RefBook : SingletonMonoBehaviour<RefBook>
{
    ...
    public static bool CheckExistance<T>() => Instance.CheckExistanceInternal(typeof(T));
    public static bool CheckExistance(System.Type type) => Instance.CheckExistanceInternal(type);

    public bool CheckExistanceInternal(System.Type type)
    {
        Instance.lastAccessedType = type;

        if (!References.ContainsKey(type))
            return false;

        if (References[type].Count <= 0)
            return false;
