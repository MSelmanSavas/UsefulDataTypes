...
//Disabled due to not finding the correct assembly for the type we cant. Will need a rework here :/
...
public class TypeReferenceDrawer : PropertyDrawer
{
    ...
    public override void OnGUI(Rect position, SerializedProperty property, GUIContent label)
    {
        ...
        TypeReference typeReference = Utils.GetTargetObjectOfProperty(property) as TypeReference;
        var typeOptionsAttribute = attribute as TypeReferenceOptionsAttribute ?? null;

        if (_foundTypes.Length <= 0)
        {
            Assembly assembly = typeReference.Type != null ? Assembly.GetAssembly(typeReference.Type) : Assembly.GetAssembly(typeof(GameObject));
            ...
            if (Utils.GetTargetObjectOfProperty(property).GetType().IsGenericType)
            {
                object foundObj = Utils.GetTargetObjectOfProperty(property);
